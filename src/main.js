...
function createWindow_1(){
   const mainWindow = new BrowserWindow({width:1400,height:1050,
    ...
    function createWindow_2(){
      ...
      mainWindow2 = new BrowserWindow({width:800,height:750,
        webPreferences :{
          nodeIntegration: true,
          contextIsolation:false,
          traceProcessWarnings:true
   ...
   mainWindow2.loadFile('src/kargolist.html' );}
      ...
      const mainWindow1=""
       ...
       mainWindow1 = new BrowserWindow({width:800,height:750,
            webPreferences :{
              nodeIntegration: true,
              contextIsolation:false,
              traceProcessWarnings:true
           ...
           mainWindow1.loadFile('src/map.html' );}
...
e.sender.send('enter',(createWindow_2(),createWindow_4()))
  ...
  ipcMain.handle('new-kargo', async (e, arg) => {
        ...
        const f=JSON.parse(s)
          ...
          e.sender.send('new-kargo-created',(JSON.stringify(kargoSaved),mainWindow2.reload(),mainWindow1.reload()))
  ...
  ipcMain.handle("update-kargo", async (e, args) => {
    ...
    const updatedKargo = await kargo.findByIdAndUpdate(
      args.idToUpdate,
      { clientName: args.clientName, address: args.address,lat:args.lat, lng:args.lng, teslimDurumu:args.teslimDurumu }
     ).exec();
    console.log(updatedKargo);
    e.sender.send("update-kargo-success", (JSON.stringify(updatedKargo),mainWindow1.reload(),mainWindow2.reload()));
  });
...
console.log(data.response)
      ...
      e.sender.send("delete-kargo-success", (JSON.stringify(kargoDeleted),mainWindow2.reload(),mainWindow1.reload())); 
...
ipcMain.handle('new-client', async (e, arg) => {
  ...
  const f=JSON.parse(s)
   ...
   const tasks= await kargo.find({ teslimDurumu: /Teslim Edilmedi/i }, 'lat lng').exec();
   var number=""
   var z=[]
...
z.push({lat:tasks[i].lat,lng:tasks[i].lng,number:number})
  ...
  ipcMain.handle('teslim', async (e, arg) => {
    const s = JSON.stringify(arg)
    const f=JSON.parse(s)

    const filter = { lat:f.lat, lng:f.lng };
    const update = { teslimDurumu:"Teslim Edildi" };
    
  const updatedTask = await kargo.findOneAndUpdate(filter, update);
  
   e.sender.send("teslim-durumu", (JSON.stringify(updatedTask),mainWindow2.reload()));
